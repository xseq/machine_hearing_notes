% This is to discuss Human and Machine Hearing book fig. 5.4: STFT


clear; clc;
[y, fs] = audioread('../wav_files/dream.wav');   % or any speech wav file
[n_samples, ~] = size(y);
...
window = hamming(hamming_len);    % window in time domain
...
for p = 1 : n_frames
    idx_start = (p - 1) * hop_sample + 1;
    idx_end = idx_start + hamming_len - 1;
    frame_data = y(idx_start : idx_end, 1);
    windowed_data = frame_data .* window;    % windowing
...
stft_abs = abs(stft_stack);   % amplitude only
...
ceps_array = uint8((double(ceps_real_stack) ./ smooth_maxv) .* 255);
ceps_array = flip(ceps_array, 1);
ceps_mask = uint8(ceps_array > 150);
ceps_array = ceps_array .* ceps_mask;
subplot(2, 1, 2)
image(ceps_array);
colormap(gray(255));
